...
class TestSubscriptionStore:
    def test_set_get_roundtrip_persists_across_instances(self, tmp_path: Path) -> None:
        ...
        push.SubscriptionStore(path).set("dev-1", SUB_A)
        # a fresh instance simulates a server restart
        assert push.SubscriptionStore(path).get("dev-1") == SUB_A

    def test_remove_persists(self, tmp_path: Path) -> None:
        path = tmp_path / "subs.json"
        store = push.SubscriptionStore(path)
        store.set("dev-1", SUB_A)
        assert store.remove("dev-1") is True
        assert store.remove("dev-1") is False  # already gone
        assert push.SubscriptionStore(path).get("dev-1") is None

    def test_missing_file_means_empty_store(self, tmp_path: Path) -> None:
        assert push.SubscriptionStore(tmp_path / "nope.json").get("dev-1") is None
    ...
    def test_resubscribe_after_relogin_drops_the_orphaned_id(self, tmp_path: Path) -> None:
        ...
        store = push.SubscriptionStore(tmp_path / "subs.json")
